...
def test_memory_runtime_route_derives_stable_mcp_url() -> None:
    first = memory_runtime_route_for_realm("r:benchmark:default", base_port=8030)
    second = memory_runtime_route_for_realm("r:benchmark:default", base_port=8030)

    assert first == second
    assert first.mcp_http_url == f"http://127.0.0.1:{first.mcp_port}/mcp"
    assert 8030 <= first.mcp_port <= 10029


def test_stable_memory_realm_port_avoids_used_ports() -> None:
    first = stable_memory_realm_port("r:benchmark:default", base_port=8030)
